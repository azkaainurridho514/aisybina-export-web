...
<head>
  ...
  <script>
    window.ADMIN_API_BASE = @json(url('/admin'));
    window.ADMIN_LOGIN_URL = @json(url('/login-admin-aisybina-export'));
    window.ADMIN_PAGE_URL = @json(url('/admin'));
  </script>
  ...
  <style>
    ...
    .admin-page-number.active {
      background: var(--forest, #1f3b2e);
      color: #fff;
      border-color: var(--forest, #1f3b2e);
    }
    .admin-pagination .btn-icon:disabled {
      opacity: .45;
      cursor: not-allowed;
    }
  </style>
